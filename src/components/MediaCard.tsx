...
export const MediaCard: React.FC<MediaCardProps> = ({
  ...
  const getStatusBadge = () => {
    switch (userWatchStatus) {
      case 'watching':
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-medium bg-amber-500 text-slate-950 px-2 py-0.5 rounded-md font-semibold shadow-md">
            <Eye className="w-3 h-3" /> İzliyorum
          </span>
        );
      case 'plan_to_watch':
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-medium bg-blue-500 text-white px-2 py-0.5 rounded-md shadow-md">
            <Clock className="w-3 h-3" /> İzleyeceğim
          </span>
        );
      case 'watched':
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-medium bg-emerald-500 text-slate-950 px-2 py-0.5 rounded-md font-semibold shadow-md">
            <Check className="w-3 h-3" /> İzlendi
          </span>
        );
      default:
        return null;
